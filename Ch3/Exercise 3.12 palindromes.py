...
def main(): 
    ...
    for num in range(10000, 0, -1):
        if num == count:
            num_to_display = int(user_num // count)
            user_num = user_num % count
            
            count /= 10

            num_lst.append(num_to_display)
            num_as_str += str(num_to_display)
            #print(int(num_to_display), end = ' ')

    if num_lst[0] == num_lst[4]:
        if num_lst[1] == num_lst[3]:
            print(f'{num_as_str} is a palindrome')
    else:
        print(f'{num_as_str} is not a palindrome')
